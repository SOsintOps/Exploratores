...
- **New:** Catalogue export: `assets/data/search-library.json` (pure-data copy of the catalogue) and `assets/data/catalogue-meta.json` (version, date, entry and disabled counts, SHA-256, download URL), generated by `scripts/catalogue/build-meta.mjs` and guarded by `verify-meta.mjs` in the CI. Groundwork for the optional in-browser catalogue update planned for a later release.
- **Improved:** Tests: new `tests/linkcheck.test.mjs` (19 cases) covering the disable/re-enable logic, line-ending preservation and the export hashing; the CI runs it next to the validators suite.
- **New:** FAQ: "What happens to the tools on a dead host?" entry; Customise guide documents the optional `no_input` and `disabled` catalogue fields.
